...
# The fork-data root contributes its leading 28 bytes to a 32-byte domain.
# The first 4 bytes of the domain are the domain type, so 28 bytes remain.
...
def compute_signing_root(ssz_object: SSZType, domain: Domain) -> Root:
