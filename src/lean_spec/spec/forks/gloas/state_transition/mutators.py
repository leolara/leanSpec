...
def get_consolidation_churn_limit(state: BeaconState) -> Gwei:
    ...
    churn = int(get_total_active_balance(state)) // int(CONSOLIDATION_CHURN_LIMIT_QUOTIENT)
    return Gwei(churn - churn % int(EFFECTIVE_BALANCE_INCREMENT))
...
def compute_consolidation_epoch_and_update_churn(
    ...
    per_epoch_churn = int(get_consolidation_churn_limit(state))
...
def queue_excess_active_balance(state: BeaconState, index: ValidatorIndex) -> BeaconState:
    """
...
def switch_to_compounding_validator(state: BeaconState, index: ValidatorIndex) -> BeaconState:
    ...
    state = replace_validator(state, int(index), updated_validator)
    return queue_excess_active_balance(state, index)
